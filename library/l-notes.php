...
<head>
  ...
  $(document).ready(function(){
    $("#btn-save-notes").click(function(){
      ...
      if(title==""){
        ...
        if(notes==""){
          ...
          $.post("actions/notes/l-action-editNotes.php",{postId:id,postTitle:title,postNotes:notes},function(edit){
            $("#msg").html(edit);
            window.location.replace("l-notes.php");
          });
        }
      }
    ...
    $("#btn-add-notes").click(function(){
      ...
      if(title==""){
        ...
        if(notes==""){
          ...
          $("#msg").html("<div class='alert alert-info'><strong>Saving... </strong>Please wait..</div>");
          $.post("actions/notes/l-action-addNotes.php",{postTitle:title,postNotes:notes},function(add){
            $("#msg").html(add);
            window.location.replace("l-notes.php");
          });
        }
    ...
    $("#btn-delete-notes").click(function(){
      ...
      $.post("actions/notes/l-action-deleteNotes.php",{postId:id},function(del){
        $("#msg").html(del);
        window.location.replace("l-notes.php");
      });
    });
    ...
    <div id="wrapper">
      ...
      if(isset($_GET['action'])){
          $id = $_GET['id'];
            ...
            $mynotes = $db->query("select * from lib_notes where id='$id' ") or die("Can't complete process. Please contact your Administrator.");
...
<div id='msg'></div>
            ...
            <div class="row">
              ...
              echo "
                    <p class='text-right'>
                    <a href='l-notes.php'  class='btn btn-default'>New</a>
          ...
          <div class="panel panel-info">
              ...
              <div class="panel-body">
                  <div class="list-group">
                    <?php
                        $mynotes = $db->query("select * from lib_notes where stat=1 order by id desc limit 30") or die("Can't complete process. Please contact your Administrator.");
                        if($mynotes->num_rows!=0)
                        {
                          while($rows=$mynotes->fetch_assoc())
                          {
                            ...
                            echo "<a href='l-notes.php?action=edit&id=$id' class='list-group-item'>
                                $title
