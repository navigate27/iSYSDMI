...
<head>
  ...
  $(document).ready(function(){
    $("#btn-save-notes").click(function(){
      ...
      if(title==""){
        ...
        if(notes==""){
          ...
          $.post("actions/gr-action-editNotes.php",{postId:id,postTitle:title,postNotes:notes},function(edit){
            $("#msg").html(edit);
          });
        }
      }
    ...
    $("#btn-add-notes").click(function(){
      ...
      if(title==""){
        ...
        if(notes==""){
          ...
          $("#msg").html("<div class='alert alert-info'><strong>Saving... </strong>Please wait..</div>");
          $.post("actions/gr-action-addNotes.php",{postTitle:title,postNotes:notes},function(add){
            window.location.replace("gr-notes.php");
            $("#msg").html(add);
          });
        }
      }
    ...
    $("#btn-delete-notes").click(function(){
      ...
      $.post("actions/gr-action-deleteNotes.php",{postId:id},function(del){
        window.location.replace("gr-notes.php");
        $("#msg").html(del);
      });
    });
    ...
    <div id="wrapper">
      ...
      if(isset($_GET['action'])){
          $id = $_GET['id'];
            ...
            $mynotes = $db->query("select * from gr_notes where id='$id' and tnum='$user' ") or die("Can't complete process. Please contact your Administrator.");
...
<div id='msg'></div>
            ...
            <div class="row">
              ...
              echo "
                    <p class='text-right'>
                    <a href='gr-notes.php'  class='btn btn-default'>New</a>
          ...
          <div class="panel panel-info">
              ...
              <div class="panel-body">
                  <div class="list-group">
                    <?php
                        $mynotes = $db->query("select * from gr_notes where stat=1 and tnum='$user' order by date desc limit 30") or die("Can't complete process. Please contact your Administrator.");
                        if($mynotes->num_rows!=0)
                        {
                          while($rows=$mynotes->fetch_assoc())
                          {
                            ...
                            echo "<a href='gr-notes.php?action=edit&id=$id' class='list-group-item'>
                                $title
